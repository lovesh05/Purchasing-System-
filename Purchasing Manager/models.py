...
class PurchaseOrder(models.Model):
    ...
    purchaserID = models.ForeignKey(
        Purchaser, on_delete=models.SET(get_purchaserID))
    vendorID = models.ForeignKey(Vendor, on_delete=models.SET(get_vendorID))
    poValidDate = models.DateField(max_length=10)
    poApprovalStatus = models.CharField(
        default='PENDING', choices=STATUS_CHOICES, max_length=10)
...
class QuotationProduct(models.Model):
    ...
    # quotationID = models.ForeignKey(Quotation, on_delete=models.SET(get_quotationID))
    purchaserID = models.ForeignKey(
        Purchaser, on_delete=models.SET(get_purchaserID), null=True)
    vendorID = models.ForeignKey(Vendor, on_delete=models.SET(get_vendorID))
...
class Quotation(models.Model):
    ...
    quotationStatus = models.CharField(
        default='PENDING', choices=STATUS_CHOICES, max_length=10)
