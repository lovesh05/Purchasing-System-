...
def addquotation(request):
    ...
    if request.method == 'POST':
        ...
        newquotation_purchaser = Purchaser.objects.get(pk=request.POST.get('purchaserID'))
        newquotation_vendor = Vendor.objects.get(pk=request.POST.get('vendorID'))
        newquotation_total = request.POST.get('totalPrice')
        products = request.POST.get('products')
        products_array = json.loads(products)

        newquotation = Quotation(quotationDate = newquotation_date, quotationValidDate =
        newquotationvalid_date, purchaserID = newquotation_purchaser, vendorID = newquotation_vendor,
...
def approvequotation(request, quotation_id):

    quotation_object = Quotation.objects.get(pk=quotation_id)
    quotation_object.quotationStatus = 'APPROVED'
    ...
    return render(request,'addquotation/approvedquotation.html',context)

def approvequotationlist(request):
    ...
    return render(request,'addquotation/approvequotationlist.html',context)

def viewselectedquotationlist(request):

    quotationList = Quotation.objects.filter(quotationStatus='APPROVED')
    context = {
        'quotationList' : quotationList,
    }
    
    return render(request,'addquotation/viewselectedquotationlist.html',context)
